...
try:
    ...
    with open('status.dat', 'rb') as f:
        ...
        if 'y' in lol:
            ...
            scraped_grp = input(f'{INPUT}{cy} Ссылка на чат для стила сабов: {r}')
            index = 0
except:
    scraped_grp = input(f'{INPUT}{cy} Ссылка на чат для стила сабов: {r}')
    index = 0
...
print(f'{info}{lg} Количество привязанных акков: {w}{len(accounts)}')
number_of_accs = int(input(f'{INPUT}{cy} Количество аккаунтов для использования в скрипте: {r}'))
print(f'{info}{cy} Выберете опцию{lg}')
print(f'{cy}[0]{lg} Добавить в публичную группу')
print(f'{cy}[1]{lg} Добавить в приватную группу')
choice = int(input(f'{INPUT}{cy} Напишите ваш выбор: {r}'))
if choice == 0:
    target = str(input(f'{INPUT}{cy} Ссылка на публичную группу: {r}'))
else:
    target = str(input(f'{INPUT}{cy} Ссылка на приватную группу: {r}'))
print(f'{grey}_'*50)
#status_choice = str(input(f'{INPUT}{cy} Вы хотите добавить активных участников?[y/n]: {r}'))
